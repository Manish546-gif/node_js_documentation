...
All examples below assume `var fs = require('fs');` and that the referenced
...
### fs.chown(path, uid, gid, callback)
    ...
    fs.chown('file.txt', 1000, 1000, function (err) {
      if (err) throw err;
    });
...
### fs.lchown(path, uid, gid, callback)
    ...
    fs.lchown('link.txt', 1000, 1000, function (err) {
      if (err) throw err;
    });

### fs.lchownSync(path, uid, gid)

    fs.lchownSync('link.txt', 1000, 1000);

### fs.chmod(path, mode, callback)

    fs.chmod('file.txt', 0644, function (err) {
      if (err) throw err;
    });
...
### fs.lchmod(path, mode, callback)
    ...
    fs.lchmod('link.txt', 0755, function (err) {
      if (err) throw err;
    });

### fs.lchmodSync(path, mode)

    fs.lchmodSync('link.txt', 0755);
...
### fs.lstat(path, callback)
    ...
    fs.lstat('link.txt', function (err, stats) {
      if (err) throw err;
...
### fs.lstatSync(path)
    ...
    var stats = fs.lstatSync('link.txt');
    console.log(stats.isSymbolicLink());
...
### fs.link(srcpath, dstpath, callback)
    ...
    fs.link('file.txt', 'hardlink.txt', function (err) {
      if (err) throw err;
    });
...
### fs.symlink(srcpath, dstpath[, type], callback)
    ...
    fs.symlink('file.txt', 'link.txt', 'file', function (err) {
      if (err) throw err;
    });

### fs.symlinkSync(srcpath, dstpath[, type])

    fs.symlinkSync('file.txt', 'link.txt', 'file');
...
### fs.readlinkSync(path)
    ...
    var linkString = fs.readlinkSync('link.txt');
    console.log(linkString);

### fs.realpath(path[, cache], callback)

    fs.realpath('link.txt', function (err, resolvedPath) {
      if (err) throw err;
      console.log(resolvedPath);
    });
...
### fs.realpathSync(path[, cache])
    ...
    var resolvedPath = fs.realpathSync('link.txt');
    console.log(resolvedPath);
...
### fs.rmdir(path, callback)
    ...
    fs.rmdir('empty-dir', function (err) {
      if (err) throw err;
    });
...
### fs.mkdir(path[, mode], callback)
    ...
    fs.mkdir('new-dir', 0755, function (err) {
      if (err) throw err;
    });

### fs.mkdirSync(path[, mode])

    fs.mkdirSync('new-dir', 0755);
...
### fs.utimes(path, atime, mtime, callback)
    ...
    fs.utimes('file.txt', new Date(), new Date(), function (err) {
      if (err) throw err;
    });
...
### fs.readSync(fd, buffer, offset, length, position)
    ...
    var fd = fs.openSync('file.txt', 'r');
    var buffer = Buffer.alloc(10);
    var bytesRead = fs.readSync(fd, buffer, 0, 10, 0);
    fs.closeSync(fd);
    console.log(buffer.slice(0, bytesRead).toString('utf8'));
...
### fs.watchFile(filename[, options], listener)
    ...
    fs.watchFile('file.txt', { interval: 500 }, function (curr, prev) {
...
### fs.existsSync(path)
    ...
    var exists = fs.existsSync('file.txt');
    console.log(exists);
...
### fs.createReadStream(path[, options])
    ...
    var stream = fs.createReadStream('file.txt', { encoding: 'utf8' });
